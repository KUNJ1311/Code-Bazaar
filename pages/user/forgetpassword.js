...
const ForgetPassword = () => {
	...
	const handleReset = async (e) => {
		...
		try {
			...
			const data = await fetch(`/api/user/otp/resetpassword?email=${email}`, {
				method: "GET",
			});
	...
	return (
		<>
			{!renderOTP ? (
							...
							<div className="shadow-card w-full sm:w-11/12 max-w-4xl rounded-md sm:p-6 p-2 bg-white">
								<div className="grid md:grid-cols-2 items-center md:gap-8">
									...
									<form className="flex flex-col w-full h-full justify-center pb-6 text-center bg-white rounded-3xl">
										...
										<p className="break-words mb-4 text-slate-700 sm:text-base text-sm text-center">Enter your Email to reset your password.</p>
										...
										<div className="flex justify-center">
											<button onClick={handleReset} className="w-[150px] sm:py-5 py-4 mb-5 sm:text-base text-sm font-semibold leading-none text-white transition duration-300 rounded-2xl hover:bg-primary-dark focus:ring-4 focus:ring-primary-light  bg-primary">
												Continue
											</button>
										</div>
				...
				<OTP type="resetpass" email={email} />
			)}
		</>
	);
};
