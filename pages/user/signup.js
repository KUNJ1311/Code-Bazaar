...
const Signup = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const data = await fetch(`/api/user/otp/newuser?email=${credentials.email}&name=${credentials.name}`, {
				method: "GET",
			});
	...
	return (
		<>
			{!renderOTP ? (
							...
							<div className="flex items-center mb-3">
								...
								<hr className="h-0 border-b border-solid border-gray-500 grow" />
										...
										<div className="flex justify-center">
											<button className="w-[150px] sm:py-5 py-4 mb-5 sm:text-base text-sm font-semibold leading-none text-white transition duration-300 rounded-2xl hover:bg-primary-dark focus:ring-4 focus:ring-primary-light bg-primary">Sign Up</button>
										</div>
				...
				<OTP type="newuser" email={credentials.email} password={credentials.password} name={credentials.name} />
			)}
		</>
	);
};
