...
const ChangePassword = () => {
	...
	const handleChange = async () => {
		try {
			...
			if (json.success) {
				setDetails({ oldpassword: "", newpassword: "", cnewpassword: "" });
				...
				<div className="container max-w-4xl bg-gray-50 border border-gray-200 rounded-md px-4 pt-2 lg:mx-0 mx-4 max-[300px]:mx-0 max-[300px]:px-2 mt-7 mb-8 ">
					<div className="flex flex-col">
						...
						<div className="relative flex">
							<input onChange={onChange} value={oldpassword} type="password" id="oldpassword" name="oldpassword" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-xs sm:text-sm shadow-sm outline-none focus:border-primary focus:ring-primary" placeholder="Enter your old password" />
						...
						<div className="flex flex-col sm:flex-row sm:space-x-2">
							<div className="flex flex-col flex-1">
								...
								<div className="relative flex">
									<input onChange={onChange} value={newpassword} type="password" id="newpassword" name="newpassword" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-xs sm:text-sm shadow-sm outline-none focus:border-primary focus:ring-primary" placeholder="Enter your new password" />
									<div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
										<HiLockClosed className="h-4 w-4 text-gray-400" />
									</div>
								</div>
							...
							<div className="flex flex-col flex-1">
								...
								<div className="relative flex">
									<input onChange={onChange} value={cnewpassword} type="password" id="cnewpassword" name="cnewpassword" className="w-full rounded-md border border-gray-200 px-4 py-3 pl-11 text-xs sm:text-sm shadow-sm outline-none focus:border-primary focus:ring-primary" placeholder="Confirm your new password" />
									<div className="pointer-events-none absolute inset-y-0 left-0 inline-flex items-center px-3">
										<HiLockClosed className="h-4 w-4 text-gray-400 " />
									</div>
								</div>
								{newpassword !== cnewpassword && <div className="flex md:text-sm text-xs text-red-500 font-medium">*Passwords don&apos;t match</div>}
								{newpassword && newpassword === cnewpassword && <div className="flex md:text-sm text-xs text-primary font-medium">*Passwords match</div>}
							</div>
						</div>
					</div>
