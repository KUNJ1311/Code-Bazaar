...
export async function getServerSideProps(context) {
	try {
		...
		const limit = 15;
		const totalProductsCount = await Product.aggregate([{ $match: { category: "mug" } }, { $group: { _id: "$title" } }, { $count: "count" }]);
		...
		const products = await Product.aggregate([{ $match: { category: "mug" } }, { $group: { _id: "$title", doc: { $first: "$$ROOT" } } }, { $skip: skip }, { $limit: limit }]);
